...
## Profiling Wintercraft Reborn on Linux with perf

We will be using a tool called "perf", which you can get by installing `perf` or `linux-perf` or `linux-tools-common`.

To get usable results you need to build Wintercraft Reborn with debug symbols
...
frames that took longer than usual. Wintercraft Reborn already contains annotations for
its frames.
...
Tracy consists of a client (Wintercraft Reborn) and a server (the gui).

Install the server, e.g. using your package manager.

### Building

Build Wintercraft Reborn with `-DDBUILD_WITH_TRACY=1`, this will fetch Tracy for building
...
Start the Tracy server and Wintercraft Reborn. You should see Wintercraft Reborn in the menu.
...
If built with Tracy, Wintercraft Reborn loads its API in the global `tracy` table.
